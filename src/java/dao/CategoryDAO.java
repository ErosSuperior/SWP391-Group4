...
public class CategoryDAO {
    ...
    public void addCategory(String categoryName, String icon) {
        Connection conn = null;
        PreparedStatement ps = null;
        String sql = "INSERT INTO categories (category_name, icon) VALUES (?, ?)";
        try {
            conn = new DBContext().getConnection();
            ps = conn.prepareStatement(sql);
            ps.setString(1, categoryName);
    ...
    public void updateCategory(int categoryId, String categoryName, String icon) {
        Connection conn = null;
        PreparedStatement ps = null;
        String sql = "UPDATE categories SET category_name = ?, icon = ? WHERE category_id = ?";
        try {
            conn = new DBContext().getConnection();
            ps = conn.prepareStatement(sql);
            ps.setString(1, categoryName);
    ...
    public int countCategories(String nameOrId) throws Exception {
        int count = 0;
        StringBuilder query = new StringBuilder("SELECT COUNT(*) FROM categories WHERE 1=1 ");
